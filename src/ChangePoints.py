...
def find_change_point(data: np.array, input_data: np.array, get_feature, w: int = 15, merge_th=None):
    r"""
    ...
    while pos + w < data.shape[1]:
        ...
        err = (p_sw * 1) / np.min(data[:, pos:(pos + w)][0])
        err_list.append(err)
        
        if last is not None:
            if (abs(err) > 1) and tail_len == 0:
